...
do
	...
	if not title or (IsAddOnLoaded(SourceName) == false) then
		loaded = false
		return
	end
end
	...
	if type(GatherMate2.gmdbs[db_type]) == "table" then
		...
		for loc, t in pairs(GatherMate2.gmdbs[db_type][zoneID]) do
			...
			if t == node_type then
				...
				local x, y, l = Routes.mapData:DecodeLoc(loc) -- ignore level for now
...
local function InsertNode(event, zone, nodeType, coord, node_name)
	-- Convert coords
	local x, y, l = Routes.mapData:DecodeLoc(coord) -- ignore level for now
	local newCoord = Routes:getID(x, y)
	-- Convert zone
	local zoneLocalized = Routes.mapData:MapLocalize(zone)
