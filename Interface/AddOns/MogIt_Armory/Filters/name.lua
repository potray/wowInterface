...
local f = mog:CreateFilter("armory_player");
local searchString;
...
f.edit = CreateFrame("EditBox","MogItArmoryFiltersPlayer",f,"SearchBoxTemplate");
f.edit:SetHeight(16);
...
function f.Filter(player)
	if searchString:trim() == "" then
		return true
	end
	return player and (player:lower():find(searchString, nil, true));
end
