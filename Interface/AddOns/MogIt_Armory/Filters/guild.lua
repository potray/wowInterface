...
local f = mog:CreateFilter("armory_guild");
local searchString;
...
f.label:SetText(GUILD..":");
f.label:SetJustifyH("LEFT");

f.edit = CreateFrame("EditBox","MogItArmoryFiltersGuild",f,"SearchBoxTemplate");
f.edit:SetHeight(16);
...
function f.Filter(guild)
	if searchString:trim() == "" then
		return true
	end
	return guild and (guild:lower():find(searchString, nil, true));
end
